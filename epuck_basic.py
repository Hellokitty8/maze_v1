...
from numpy import *
from math import *              #   "            "
              # An extra Python module (that you'll have to download)
           # A module provided by Keith Downing for this assignment
...
class EpuckBasic (DifferentialWheels):
  ...
  num_dist_sensors = 5
  num_light_sensors = 8
  encoder_resolution = 118 # for wheel encoders
...
# something similar in order to get access to camera and sensor data, along with the timestep.
  ...
  def basic_setup(self, tempo = 1.0):
      ...
      self.dist_sensors = [self.getDistanceSensor('ds'+str(x)) for x in range(self.num_dist_sensors)]  # distance sensors
...
# activate the bot for a brief period (with the current motor settings in effect).
  ...
  def getCompassValue(self):   
      ...
      cps_values=self.compass.getValues()
      return cps_values
...
# from which basic_epuck inherits.  A typical value for RES is around 150. 
  ...
  def spin_angle(self, angle):
      ...
      current=(atan2(north[0], north[1])-pi/2)/pi*180
      if (current < 0.0):
      ...
      angle_f=current-a
      if ( angle_f < 0.0):
           angle_f = angle_f + 360.0
           
      if ( angle_f > 360.0):
           angle_f = angle_f - 360.0
           
      lis= [0.0,90.0,180.0,270.0,360.0]   
      mid=abs(array(lis)-angle_f)
      angle_f=lis[argmin(mid)]

      if angle_f >359.3 or angle_f <0.7:
          while 0.7<current<359.3:
              if angle>0: self.setSpeed(-50,50) 
              else:self.setSpeed(50,-50) 
              north=[self.getCompassValue()[0],self.getCompassValue()[2]]
              current=(atan2(north[0], north[1])-pi/2)/pi*180
              if (current < 0.0):
                  current = current + 360.0
          ...
          while current<angle_f-0.7 or current>angle_f+0.7:
              if angle>0: self.setSpeed(-50,50) 
              else:self.setSpeed(50,-50) 
              north=[self.getCompassValue()[0],self.getCompassValue()[2]]
              current=(atan2(north[0], north[1])-pi/2)/pi*180
...
# ****** Accessories ******
  ...
  def get_Accelerometers(self):
      ...
      AcmVal=self.accelerometer.getValues()
      return AcmVal
...
# snapshots, etc.
  ...
  def calculate_treshold(self,sensor):
      sensor_val=[int(i>90) for i in sensor]
      return sensor_val
